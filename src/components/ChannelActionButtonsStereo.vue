<template>
  <div class="btn-sec chn-act-stero-btn btn-mute">
    <div class="muteIcon">
      <span v-if="!isActiveMute" class="icon iconfont" @click="toggleMute"
        >&#xe61d;</span
      >
      <span v-else class="icon iconfont disabled" @click="toggleMute"
        >&#xe69d;</span
      >
    </div>
  </div>
</template>
<script>
import { WebRtcClient } from "../webRTC/localClient";
import { mapState } from "vuex";
import common from "../mixins/common";
export default {
  mixins: [common],
  props: {
    previewId: {
      type: String,
    },
    channelIndex: {
      type: Number,
    },
    isMute: {
      type: Boolean,
    },
  },
  data() {
    return {
      isActiveMute: false,
    };
  },
  mounted() {
    this.initMute();
  },
  updated() {
    var muteButtons = document.getElementsByClassName("muteButton");
    if (document.getElementsByClassName("follow").length >= 1) {
      for (var k = 0; k < muteButtons.length; k++) {
        muteButtons[k].disabled = true;
      }
    }
  },
  computed: {
    ...mapState({
      audioSourceData: (state) => state.audioSource.audioSourceData.response,
      forceUpdateKey: (state) => state.audioSource.forceUpdateKey,
      groupMute: (state) => state.audioSource.groupMute,
      advanceMix: (state) => state.audioSource.advanceMix,
      selectedPairValueForPGM: (state) =>
        state.audioSource.selectedPairValueForPGM,
    }),
  },
  watch: {
    forceUpdateKey() {
      this.initMute();
    },
    audioSourceData() {
      this.initMute();
    },
    selectedPairValueForPGM() {
      this.initMute();
    },
    groupMute() {
      if (this.groupMute.mute) {
        if (
          this.previewId == this.groupMute.previewId &&
          this.channelIndex == this.groupMute.channelIndex
        ) {
          this.toggleMute();
        }
      }
    },
  },
  methods: {
    initMute() {
      let self = this;
      this.audioSourceData.Sources.forEach((element, idx) => {
        element.Channels.forEach((item, index) => {
          if (self.advanceMix) {
            if (
              (element.PairSelected == 1 && (index == 0 || index == 1)) ||
              (element.PairSelected == 2 && (index == 2 || index == 3)) ||
              (element.PairSelected == 3 && (index == 4 || index == 5)) ||
              (element.PairSelected == 4 && (index == 6 || index == 7))
            ) {
              if (self.previewId == element.PreviewID) {
                self.isActiveMute = item.isMute;
                if (idx == 0) {
                  // affect only source 1
                  WebRtcClient.setChannelSources(self.previewId, index, item);
                  WebRtcClient.changeGainLeft(self.previewId);
                  WebRtcClient.changeGainRight(self.previewId);
                }
              }
            }
          } else {
            if (
              self.previewId == element.PreviewID &&
              self.channelIndex == index
            ) {
              self.isActiveMute = item.isMute;
            }
          }
        });
      });
    },
    toggleMute() {
      this.isActiveMute = !this.isActiveMute;
      let self = this;
      this.audioSourceData.Sources.map((item) => {
        if (item.PreviewID == self.previewId) {
          if (self.advanceMix) {
            item.Channels.map((element, index) => {
              if (
                (item.PairSelected == 1 && (index == 0 || index == 1)) ||
                (item.PairSelected == 2 && (index == 2 || index == 3)) ||
                (item.PairSelected == 3 && (index == 4 || index == 5)) ||
                (item.PairSelected == 4 && (index == 6 || index == 7))
              ) {
                element.isMute = self.isActiveMute ? true : false;
                WebRtcClient.setChannelSources(self.previewId, index, element);
              }
            });
            WebRtcClient.changeGainLeft(self.previewId);
            WebRtcClient.changeGainRight(self.previewId);
            this.$store.dispatch(
              "audioSource/actionSetGainValue",
              this.audioSourceData
            );
          } else {
            item.Channels.map((element, index) => {
              element.isMute = self.isActiveMute ? true : false;
              WebRtcClient.setChannelSources(self.previewId, index, element);
            });
            WebRtcClient.changeGainLeft(self.previewId);
            WebRtcClient.changeGainRight(self.previewId);
            this.$store.dispatch(
              "audioSource/actionSetGainValue",
              this.audioSourceData
            );
          }
        }
      });
    },
  },
};
</script>
