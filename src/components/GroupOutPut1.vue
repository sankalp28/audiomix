      ...
      <div class="btn-sec on-off">
        <span>
          <button
            :class="onButtonClass()"
            v-on="
              (this.audioGroupData.Groups[0].ChannelIds.length ||
                isGroupEnabled) &&
              !this.afvEnabled()
...
export default {
  ...
  mounted() {
    this.isGroupEnabled = this.audioGroupData.Groups[0].isEnabled;
    this.isActiveMute = this.audioGroupData.Groups[0].isMute;
    this.gainValue = this.gain;
  },
  computed: {
    ...mapState({
      volumeForGroups: (state) => state.audioSource.volumeForGroup1,
  ...
  methods: {
    onButtonClass() {
      ...
      } else if (this.audioGroupData.Groups[0].ChannelIds.length) {
        this.onButtonDisabled = false;
    ...
    handleToggleGroupEnable() {
      ...
      this.audioGroupData.Groups[0].isEnabled = this.isGroupEnabled;
      this.setDataInGroupsAPIWithoutCommit(this.audioGroupData);
    },
    handleGaincrementValuehange: _.throttle(function() {
      let self = this;
      this.audioGroupData.Groups[0].Gain = this.gainValue;
      WebRtcClient.setGroupData(this.audioGroupData.Groups[0], 1);

      this.audioGroupData.Groups[0].ChannelIds.map((element) => {
    ...
    toggleMute() {
      ...
      this.audioGroupData.Groups[0].isMute = this.isActiveMute;
      ...
      this.audioGroupData.Groups[0].ChannelIds.map((element) => {
        ...
        // mute/unmute all sources which are added to group 1
